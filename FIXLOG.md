...
## 2026-09-04 (fourth pass) — An eighth real CV, reported directly by the user with two docx files (a ChatGPT-produced version and the portal's own output): a whole "till date" convention never recognised at all, splitting a role's date across two lines, and a present/previous rule that only ever worked for one specific employer name

Reported directly, with two `.docx` files: one produced by ChatGPT from the person's information, one produced by this portal from the same person's information. Comparing them showed the portal's version missing an entire Present Employment section, the wrong job title in the letterhead ("Administrator Mar 2019 – Apr 2020" -- a past, closed role -- instead of her real current title), and, as a direct downstream consequence, an auto-drafted biography built from that same wrong title ("Dr Afiya Thaha is an Administrator Mar 2019 – Apr 2020, with over 8 years of experience..."). Neither file is the actual raw source document originally uploaded (both are already-rewritten/already-generated versions), so root-causing was done by reconstructing the shape of the underlying text from what both versions had in common, and every fix below was verified against that reconstruction plus the full regression suite -- not by hand-patching this person's specific content. No live-upload verification was possible for this CV, since no genuine raw source file was available; this is noted explicitly rather than treated as done. Regression tests in `app/backend/test_till_date_and_present_role_detection.py`. Suite: 66/75 throughout (9 pre-existing failures, unchanged).

| Defect | Cause | Fix |
|---|---|---|
| An entry dated only "September 2024 till date" never matched any date range at all, so it could never be recognised as an ongoing, current role | `YEAR_RANGE_RE`'s separator group and `PRESENT_ROLE_RE` both recognised "to date" as an open-ended marker, but not "till date" -- a real, common synonym this CV used throughout instead | Added "till" as a recognised separator/synonym alongside "to" in both regexes |
| "Executive, Director (2024)" / "till date Spearheaded the international expansion..." -- one role, with its start year and its open-ended marker landed on two different physical lines -- was split into two bogus, unrelated entries | `ENTRY_END_YEAR_RE`'s "this entry ends with its own date column" check correctly reads "(2024)" alone as a complete, CLOSED single-year date, forcing a new entry to start at the very next line -- with no way to tell that shape apart from a genuinely closed role followed by an unrelated new one | New `OPEN_ENDED_MARKER_LEAD_RE`: a following line that itself OPENS with an open-ended marker ("till date", "present", "onwards", ...) is now the narrow exception that keeps the two lines merged into one entry instead |
| Even once merged onto one line, "(2024) till date" still failed to parse as a date range at all | The closing paren sitting directly between the start year and the separator word broke `YEAR_RANGE_RE`'s match immediately, before it ever reached the separator | Added an optional `\)?` immediately after the start-date capture group |
| Every genuinely current, concurrent role EXCEPT the one at Middlesex University was filed under Previous Employment instead of Present -- this person holds three simultaneous ongoing roles, only one of them at Middlesex | Present-vs-previous classification, for a CV with one undifferentiated "EXPERIENCE" heading and no separate Present/Previous headings, required an entry's employer name to literally contain the substring "middlesex" -- a hardcoded assumption from an earlier fix this session that never accounted for more than one concurrent role existing at all | Classification now follows `fields["is_current"]` directly -- the real, explicit signal `_extract_employment_fields` already sets whenever an actual open-ended marker was found in the text -- with the employer-name substring check dropped entirely |
| *(regression, caught before shipping)* A person with exactly ONE real current role, but several duty bullets under it, had their job title wrongly deferred to the letterhead instead of the (correct, more detailed) title from their Present Employment entry | `_promote_present_role`'s ambiguity check (added in an earlier pass this session, for a genuinely-multi-role CV) counted every ITEM filed under `present_employment` to decide "is this person's present employment ambiguous" -- and the fix directly above this one correctly made a role's own duty bullets inherit `present_employment` too, which inflated a single role with three duty bullets into what looked like four separate concurrent roles | The count now only includes items that actually carry a real `fields["title"]` -- i.e. genuine role/title lines -- not duty-bullet items, which carry no fields of their own |

## 2026-09-04 (third pass) — A seventh real CV, reported directly by the user with a screenshot: a mid-sentence bullet split, an unrecognised arrow bullet glyph, and a regression caught mid-fix

Reported directly, with a screenshot showing a duty sentence broken across two bullets ("...student's graduation" / "projects.") and a Skills line glued onto an unrelated "PERSONAL INFO" heading ("Languages: English | Arabic ➢ PERSONAL INFO"). Fixing the second issue (an unrecognised arrow bullet glyph) introduced a real regression, caught by the corpus suite before it shipped and fixed in the same pass. Regression tests in `app/backend/test_wrapped_bullets_and_arrow_glyphs.py`. Suite: 57/66 throughout (9 pre-existing failures, confirmed via `git stash` to predate this change).

| Defect | Cause | Fix |
|---|---|---|
| A duty sentence was published broken across two bullets: "...student's graduation" as one bullet, "projects." as the next | The source document itself has the sentence split across two real bullet-marked lines (a bad export, not a genuine second responsibility) -- bulleted-mode grouping always starts a new item at every bullet, with no concept of "this bullet doesn't look like a complete thought" | A SHORT bulleted line (≤30 chars) that opens lowercase, directly after an accumulated line with no terminal punctuation, is now treated as the back half of the same sentence and merged rather than started as a new item. Capped to short fragments specifically: a long lowercase-opening bullet is far more often a real, separate duty, and an earlier, uncapped version of this fix merged one on and welded together several already only loosely-bounded entries elsewhere in a different real CV, producing text no longer verbatim in the source (caught by the corpus suite, not shipped) |
| "Languages: English \| Arabic ➢ PERSONAL INFO" -- a Skills line with an entire unrelated heading glued onto its end | The bullet in front of "PERSONAL INFO" is a Wingdings-style arrow glyph ("➢"), not one of the characters `BULLET_CHARS` recognised -- unrecognised, the line read as plain text continuing whatever came before it | Added `➢➤➣❖▶▷` to `BULLET_CHARS` |
| *(regression, caught before shipping)* Every ordinary ASCII "-" bullet in the corpus stopped being recognised as a bullet at all | The new arrow characters were appended AFTER the literal hyphen already in `BULLET_CHARS`. Inside a regex character class, a "-" that isn't at the very start or end is a RANGE operator, not a literal character -- "—-➢" silently became an unintended Unicode range from em dash through the arrow glyph instead of three literal characters, and a plain ASCII hyphen no longer fell inside it | Moved the hyphen to the very end of the character class, the one position with no range ambiguity |
| A single word ending a real sentence ("...evaluating student's graduation **projects.**") was forcing a spurious mid-sentence item boundary | `_find_heading_key`'s exact-synonym match stage has no shape gate at all (that check is applied only to the LOOSER fallback stages) -- "PROJECTS" is a real, legitimate synonym for the "grants" heading, and a normalized, period-stripped "projects." matched it exactly even though nothing about a lowercase word ending a sentence looks like a heading | `_find_heading_key` now rejects any candidate whose original text ends in a bare "." before any match stage runs at all -- no heading in this template, official or synonym, ever ends in one |

## 2026-09-04 (second pass) — A sixth real CV, reported directly by the user pasting the full raw Experience section and the portal's output: five employment-parsing defects on a "Title | Employer | Location" convention

Reported directly, pasting a dense 11-role Experience section verbatim alongside the generated output. Every single entry's title/employer had been fused into one fabricated field, a "|Contract" tag had become a fabricated job title, a title starting with a gerund-shaped word ("Teaching Fellow") had its own date line split off into a bogus entry, "Economist" welded onto the previous job's last duty, and -- the most consequential -- every duty bullet under the person's CURRENT role had silently reappeared under Previous Employment instead, because the whole CV uses one undifferentiated "EXPERIENCE" heading with no separate Present/Previous headings to lean on. Regression tests in `app/backend/test_pipe_delimited_employment.py`. Suite: 55/62 throughout (7 pre-existing failures, confirmed via `git stash` to predate this change).

| Defect | Cause | Fix |
|---|---|---|
| Every employment entry's title and employer were fused into one fabricated field ("Senior Lecturer \| Middlesex University Dubai \| Dubai, UAE" produced title "...\| Dubai", employer "UAE") | `_extract_employment_fields` only ever understood a comma-based "Title, Employer" split. This convention uses literal pipes for BOTH separators, with the date on its own following line rather than inside the title line -- the whole "Title \| Employer \| Location" run has at most one comma (right before the country), so the comma-split logic split it in the wrong place entirely | A new check: two or more pipe-delimited segments on the pre-date text is unambiguous -- the first is the title, everything after is the employer, with its own trailing location segment folded in the same way every other employer field on this system already carries one |
| A work-arrangement tag right after a date ("...September 2019\|Contract") became a fabricated job title, with the real title/employer (before the date) discarded entirely | With nothing else on the "after" side, "Contract" read as short, clean role text -- exactly what the "before vs after the date" heuristic prefers | The tag is now stripped from "after" before it is judged, using the same closed vocabulary (`Contract`, `Full-time`, `Part-time`, `Freelance`, `Permanent`, `Temporary`) `BARE_EMPLOYMENT_TYPE_RE` already uses for an identical tag sitting alone on its own line |
| A job title starting with a gerund-shaped word ("Teaching Fellow \| Queen Mary University of London \| London, UK") had its own date line split away into a separate, title-less bogus entry | `_starts_list_item`'s "a capitalised line closes a gerund list" check only looks at the SHAPE of the previous line's first word ("Teaching" matches the same pattern as a duty sentence like "Teaching computer science to..."), not whether it is actually part of a real gerund duty list | Excluded whenever the previous line contains a literal "\|" -- a real gerund duty sentence has no reason to contain one, so a pipe-delimited title line is never mistaken for the end of a duty list regardless of what its first word looks like |
| The previous job's last duty bullet welded onto the front of "Economist \| Local First CIC \| Winchester, UK" instead of it starting its own entry | "Economist" was missing from `TITLE_KEYWORDS` -- the same class of gap as "trainee" (fourth audit) | Added `economist`, plus three other plausible same-shaped gaps (`scientist`, `strategist`, `accountant`) |
| Every duty bullet under the person's CURRENT role reappeared under Previous Employment -- the title alone showed correctly under Present Employment, with none of its own duties | Under one undifferentiated "EXPERIENCE" heading (no separate Present/Previous headings), present-vs-previous is decided by checking whether an entry's own employer name contains "middlesex". A duty bullet has no employer of its own -- `_extract_employment_fields` returns `{}` for it -- so that check, run against each entry in total isolation, could never see the employer its OWN title line had just established two lines above it, and fell through to Previous every time | The classification now tracks the most recent entry that DID have real fields (a genuine title line) and has every subsequent fields-less entry (a duty bullet) inherit ITS classification, rather than being judged alone |
